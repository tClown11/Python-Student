...
class WSGIServer(object):
    ...
    scoketType = socket.SOCK_STREAM
    requestQueueSize = 5

    def __init__(self, server_address):
        #创建一个tcp套接字
        self.listenSocket = socket.socket(self.addressFamily, self.scoketType)
        #允许重复使用上次的套接字绑定的port
    ...
    def serverForever(self):
        ...
        while True:
            ...
            #因为创建的新进程中，会对这个套接字+1，所以需要在主进程中减去1次，即调用一次close
            self.clientSocket.close()

    def handleRequest(self):
        """用一个新的进程，为一个客户端进行服务"""
        recvData = self.clientSocket.recv(1024)
        requestHeaderLines = recvData.splitline()
